...
class Model(nn.Module, abc.ABC):
    ...
    def save_full(self, save_dir, model_name):
        """Saves the model to the given directory."""
        torch.save(self.state_dict(), pathlib.Path(save_dir) / model_name)

    def load(self, load_dir: Union[str, pathlib.Path]):
        """Loads the model from the given path."""
        self.load_state_dict(torch.load(pathlib.Path(load_dir) / self._MODEL_FNAME))
    
    def load_full(self, load_dir, model_name):
        """Loads the model from the given path."""
        self.load_state_dict(torch.load(pathlib.Path(load_dir) / model_name))
