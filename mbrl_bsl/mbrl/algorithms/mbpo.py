    ...
    while env_steps < cfg.overrides.num_steps:
        ...
        for steps_epoch in range(cfg.overrides.epoch_length):
            if steps_epoch == 0 or done:
                ...
                # TODO()
                if hasattr(env, "write_to_csv"):
                    env.write_to_csv(env_steps)
            ...
            if (env_steps + 1) % cfg.overrides.freq_train_model == 0:
                model_trainer.env_steps = env_steps
                ...
                if (env_steps + 1) % 5000 == 0:
                    agent.sac_agent.save_checkpoint(
                            ckpt_path=os.path.join(work_dir, "sac_%05d.pth"%(env_steps+1))
                        )
